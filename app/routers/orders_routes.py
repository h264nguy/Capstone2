...
async def checkout(request: Request) -> JSONResponse:
    ...
    # Normalize + validate
    # Also collapse duplicate drink rows before queueing. This is especially
    # important for single-pump base drinks like Water/Sprite, which should
    # behave like one normal drink order unless the user intentionally sends a
    # larger quantity.
    norm_items: List[Dict[str, Any]] = []
    merged_by_drink: Dict[str, Dict[str, Any]] = {}
    for it in items:
        ...
        if key in merged_by_drink:
            # For Water/Sprite, keep a single unit per checkout request because
            # these base drinks are meant to pulse once like a normal single item.
            if key in {"base_water", "base_sprite"}:
                merged_by_drink[key]["quantity"] = 1
            else:
                merged_by_drink[key]["quantity"] += qty
        else:
            if key in {"base_water", "base_sprite"}:
                row["quantity"] = 1
            merged_by_drink[key] = row

    norm_items = list(merged_by_drink.values())
