...
from app.core.storage import load_orders, save_orders, enqueue_esp_order, queue_position, load_esp_queue, load_drinks, load_machine_state, load_esp_done
...
async def checkout(request: Request) -> JSONResponse:
    ...
    # Collapse duplicate checkout lines before saving / queueing.
    # This especially protects single-base drinks like Water and Sprite from
    # being accidentally submitted twice by the client/UI for one checkout.
    merged_items: Dict[str, Dict[str, Any]] = {}
    for it in norm_items:
        did = str(it.get("drinkId") or "").strip()
        if not did:
            continue
        existing = merged_items.get(did)
        if existing is None:
            merged_items[did] = dict(it)
            continue

        # If the same drink appears multiple times in one payload:
        # - normal mixed drinks should combine quantities
        # - base water / base sprite should NOT double-trigger from duplicate rows,
        #   so keep the highest explicit quantity instead of summing duplicates
        if did in {"base_water", "base_sprite"}:
            try:
                existing_qty = int(existing.get("quantity", 1))
            except Exception:
                existing_qty = 1
            try:
                incoming_qty = int(it.get("quantity", 1))
            except Exception:
                incoming_qty = 1
            existing["quantity"] = max(existing_qty, incoming_qty)
        else:
            try:
                existing["quantity"] = int(existing.get("quantity", 1)) + int(it.get("quantity", 1))
            except Exception:
                existing["quantity"] = int(existing.get("quantity", 1) or 1)

        if "ratios" not in existing and isinstance(it.get("ratios"), dict):
            existing["ratios"] = it["ratios"]

    norm_items = list(merged_items.values())
