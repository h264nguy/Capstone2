...
function setMainEtaFromOrders(orders){
  ...
  if(__flushState && __flushState.flushRequired){
    box.style.display = 'block';
    bar.style.width = __flushState.flushRequested ? '100%' : '0%';
    txt.textContent = __flushState.flushRequested ? 'Waiting for tube clean-up...' : 'Waiting for flush';
...
function __renderMyQueue(box){
  ...
  if(__flushState && __flushState.flushRequired){
    const label = __flushState.flushRequested ? 'Waiting for tube clean-up...' : 'Drink complete. Press Flush Tube after grabbing your drink.';
    box.innerHTML = `
      <div class="orderRow">
        <div>
          <div class="orderTitle">Flush Status</div>
          <div class="small">Status: ${__flushState.flushRequested ? 'Flushing Requested' : 'Waiting For Flush'}</div>
          <div class="small">${label}</div>
        </div>
        <div class="orderEta">${__flushState.flushRequested ? 'Cleaning…' : 'Waiting'}</div>
      </div>
      <div class="progress"><div class="fill" style="width:${__flushState.flushRequested ? 100 : 0}%"></div></div>
    `;
    return;
  }
...
async function loadMyQueue(){
  ...
  try{
    const res = await fetch('/api/my/queue', {credentials:'include'});
    ...
    if(!orders.length && !__flushState.flushRequired){
      box.innerHTML = '<div class="small">No active orders.</div>';
      __myOrdersSnapshot = [];
      window.__myOrdersSnapshot = __myOrdersSnapshot;
      __mySnapshotTs = Date.now();
...
let __flushState = {flushRequired:false, flushRequested:false, lastDoneDrinkName:null};
window.__myOrdersSnapshot = __myOrdersSnapshot;
...
function startMyQueueAutoRefresh(){
  ...
  loadMyQueue();
  myQueueTimer = setInterval(loadMyQueue, 10000);
  if(!__localTickTimer){
