...
function runBmoSpeechQueue(){
  try{
    ...
    u.onend = () => { __bmoSpeaking = false; setTimeout(runBmoSpeechQueue, 80); };
    u.onerror = () => { __bmoSpeaking = false; setTimeout(runBmoSpeechQueue, 80); };
...
function unlockBmoSpeech(){
  ...
  try{
    const u = new SpeechSynthesisUtterance('B M O ready');
    u.volume = 0.01;
    const preferred = pickBmoVoice();
    if(preferred) u.voice = preferred;
    window.speechSynthesis.speak(u);
    setTimeout(() => { try{ window.speechSynthesis.cancel(); }catch(_e){} }, 120);
...
function parseVoiceDrinkSegments(text, mode='add'){
  ...
  parts.forEach(part => {
    ...
    const withoutQty = cleaned.replace(/^(?:\d+|one|a|an|two|three|four|five|six|seven|eight|nine|ten)\s*/i, '').trim();
