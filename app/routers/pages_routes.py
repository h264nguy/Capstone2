...
function bmoSay(text){
  try{
    ...
    unlockBmoSpeech();
    __bmoSpeechQueue.push(msg);
    if(__bmoSpeechSpeaking) return;
    const speakNext = () => {
      const next = __bmoSpeechQueue.shift();
      if(!next){ __bmoSpeechSpeaking = false; return; }
      __bmoSpeechSpeaking = true;
      try{
        ...
        const preferred = voices.find(v => /samantha|ava|victoria|karen|moira|google us english|english/i.test(v.name || ''));
        if(preferred) u.voice = preferred;
        u.onend = () => setTimeout(speakNext, 120);
        u.onerror = () => setTimeout(speakNext, 120);
        window.speechSynthesis.speak(u);
      }catch(_e){
        __bmoSpeechSpeaking = false;
      }
    };
    speakNext();
  }catch(_e){}
...
['click','touchstart'].forEach(evt => {
  window.addEventListener(evt, unlockBmoSpeech, {passive:true, once:true});
});
try{ if('speechSynthesis' in window){ window.speechSynthesis.onvoiceschanged = () => { try{ unlockBmoSpeech(); }catch(_e){} }; } }catch(_e){}
...
function addDrinkByVoice(drink, qty){
  ...
  const row = cart.find(x => x.id === drink.id);
  if(row) row.qty += qty;
  else cart.push({id:drink.id, name:drink.name, price:drink.price, calories:drink.calories, qty});
  rerenderAfterVoice();
...
function removeDrinkByVoice(drink, qty){
  ...
  row.qty -= qty;
  if(row.qty <= 0) cart = cart.filter(x => x.id !== drink.id);
  rerenderAfterVoice();
...
function splitVoiceDrinkSegments(text){
  return String(text || '')
    .split(/\s*(?:,|\band\b|\bplus\b|\bthen\b|\balso\b|\bwith\b)\s*/i)
    .map(part => normalizeVoiceText(part).trim())
    .filter(Boolean);
}

function inferQtyNearPhrase(fullText, phrase){
  const src = normalizeVoiceText(fullText);
  const target = normalizeVoiceText(phrase);
  const idx = src.indexOf(target);
  if(idx < 0) return 1;
  const before = src.slice(Math.max(0, idx - 24), idx);
  const m = before.match(/(\d+|one|a|an|two|three|four|five|six|seven|eight|nine|ten)\s*$/i);
  return m ? voiceNumberFromText(m[1]) : 1;
}

function extractExplicitQty(segment){
  const m = normalizeVoiceText(segment).match(/\b(\d+|one|a|an|two|three|four|five|six|seven|eight|nine|ten)\b/i);
  return m ? voiceNumberFromText(m[1]) : null;
}

function parseVoiceByOccurrences(text, mode='add'){
  const normalized = normalizeVoiceText(text);
  const matches = [];
  const escapeRegex = (value) => String(value || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  (drinks || []).forEach(drink => {
    buildDrinkVoiceAliases(drink).forEach(alias => {
      const target = normalizeVoiceText(alias);
      if(!target || target.length < 3) return;
      const re = new RegExp(`(^|\\s)${escapeRegex(target)}(?=\\s|$)`, 'ig');
      let m;
      while((m = re.exec(normalized)) !== null){
        const idx = m.index + (m[1] ? m[1].length : 0);
        const before = normalized.slice(0, idx);
        const tail = before.split(/(?:,|\band\b|\bplus\b|\bthen\b|\balso\b)/i).pop() || before.slice(Math.max(0, before.length - 32));
        const qty = extractExplicitQty(tail) || inferQtyNearPhrase(normalized, target) || 1;
        matches.push({idx, drink, qty, text: target, mode, kind:'match'});
      }
    });
  });
  const dedup = [];
  matches.sort((a,b) => a.idx - b.idx || b.text.length - a.text.length || a.drink.name.localeCompare(b.drink.name));
  matches.forEach(item => {
    const start = item.idx;
    const end = item.idx + String(item.text || '').length;
    const overlapIdx = dedup.findIndex(existing => existing.drink.id === item.drink.id && Math.max(existing.idx, start) < Math.min(existing.idx + String(existing.text || '').length, end));
    if(overlapIdx >= 0){
      const existing = dedup[overlapIdx];
      if(String(item.text || '').length > String(existing.text || '').length) dedup[overlapIdx] = item;
      return;
    }
    dedup.push(item);
  });
  return dedup.sort((a,b) => a.idx - b.idx);
}

function normalizeVoiceDrinkSegment(segment){
...
function parseVoiceDrinkSegments(text, mode='add'){
  const normalized = normalizeVoiceText(text);
  const occurrenceMatches = parseVoiceByOccurrences(normalized, mode);
  if(occurrenceMatches.length > 1) return occurrenceMatches;

  const parts = splitVoiceDrinkSegments(normalized);
  const out = [];
  parts.forEach(part => {
    const cleaned = normalizeVoiceDrinkSegment(part);
    if(!cleaned) return;
    const explicitQty = extractExplicitQty(cleaned);
    let qty = explicitQty || 1;
    const drink = findDrinkFromVoice(cleaned);
    const suggestion = !drink ? findDrinkSuggestion(cleaned) : null;
    if(!explicitQty && (drink || suggestion)) qty = inferQtyNearPhrase(normalized, cleaned);
    if(drink) out.push({kind:'match', drink, qty, text: cleaned, mode});
    else if(suggestion) out.push({kind:'suggestion', drink: suggestion, qty, text: cleaned, mode});
    else out.push({kind:'unknown', qty, text: cleaned, mode});
  });

  if(out.length <= 1 && occurrenceMatches.length === 1) return occurrenceMatches;
  return out;
}

function summarizeVoiceActions(items, actionWord){
...
function handleVoiceCommand(rawText){
  ...
  if(/\b(remove|delete|take away)\b/.test(text) && /\b(all|everything)\b/.test(text)){
    clearCartByVoice(heardRaw);
    return true;
  }

  const multiMode = /\b(remove|delete|take away)\b/.test(text) ? 'remove' : 'add';
  const segments = parseVoiceDrinkSegments(text, multiMode);
  if(segments.length > 1){
    const unknown = segments.filter(x => x.kind === 'unknown');
    const suggestions = segments.filter(x => x.kind === 'suggestion');
    if(!unknown.length && !suggestions.length){
      segments.forEach(item => {
        if(item.mode === 'remove') removeDrinkByVoice(item.drink, item.qty);
        else addDrinkByVoice(item.drink, item.qty);
      });
      if(status){
        const verb = multiMode === 'remove' ? 'Removed' : 'Added';
        const summary = summarizeVoiceActions(segments, verb.toLowerCase());
        status.innerHTML = `${verb} <strong>${summary}</strong>.<div style="margin-top:8px;opacity:.75;font-size:12px">Heard: ${heardRaw}</div>`;
      }
      const voiceSummary = summarizeVoiceActions(segments, multiMode);
      bmoSay(`${multiMode === 'remove' ? 'Removed' : 'Added'} ${voiceSummary}`);
      return true;
    }
  ...
  if(/\b(remove|delete|take away)\b/.test(text)){
    ...
    const suggestion = !drink ? findDrinkSuggestion(text) : null;
    if(drink){ removeDrinkByVoice(drink, qty); return true; }
    if(suggestion){ showVoiceSuggestion(suggestion, qty, 'remove', heardRaw); bmoSay(`Did you mean ${suggestion.name}`); return true; }
  ...
  if(drink){
    ...
    if(status) status.innerHTML = `Added <strong>${qty} ${drink.name}${qty>1?'s':''}</strong>.<div style="margin-top:8px;opacity:.75;font-size:12px">Heard: ${heardRaw}</div>`;
    return true;
  }
  const suggestion = findDrinkSuggestion(text);
  if(suggestion){ showVoiceSuggestion(suggestion, qty, 'add', heardRaw); bmoSay(`Did you mean ${suggestion.name}`); return true; }
