...
function parseVoiceDrinkSegments(text, mode='add'){
  ...
  if(parts.length > 1) return parts.map(part => parseSingleVoiceSegment(part, mode)).filter(Boolean);
...
// UI feedback
  ...
  try {
    ...
    if(res && res.status===404){ res = await fetch('/api/checkout', { method:'POST', headers:{'Content-Type':'application/json','Accept':'application/json'}, credentials:'include', body: JSON.stringify({items, mood: (window.currentMood && window.currentMood !== 'none') ? window.currentMood : null}) }); }


    const ct = res.headers.get("content-type") || "";
    if (res.status === 401 || res.redirected) {
      if(status) status.innerText = 'Session expired. Please log in again.';
      return;
    }
    if (!ct.includes("application/json")) {
      const txt = await res.text();
      if(status) status.innerText = txt && txt.trim() ? `Checkout failed: ${txt.slice(0,80)}` : 'Checkout failed. Please log in again.';
      return;
    }
...
async function checkout(btnEl) {
  ...
  if (!ct.includes("application/json")) {
    const txt = await res.text();
    status.innerText = txt && txt.trim() ? `Checkout failed: ${txt.slice(0,80)}` : 'Checkout failed (server returned non-JSON).';
    return;
  }
