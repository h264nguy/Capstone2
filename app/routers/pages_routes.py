...
function parseVoiceDrinkSegments(text, mode='add'){
  ...
  (drinks || []).forEach(drink => {
    const aliases = buildDrinkVoiceAliases(drink).sort((a,b)=>b.length-a.length);
    let bestAlias = null;
    for(const alias of aliases){
      const aliasText = normalizeVoiceText(alias);
      if(aliasText && body.includes(aliasText)){
        bestAlias = aliasText;
        break;
      }
    }
    if(!bestAlias) return;
    const escapedAlias = bestAlias.replace(/[.*+?^${}()|[\\]\\]/g, '\\$&');
    const qtyMatch = body.match(new RegExp(`(?:^|\\b)(\\d+|one|a|an|two|three|four|five|six|seven|eight|nine|ten)\\s+${escapedAlias}(?:\\b|$)`, 'i'));
    const qty = qtyMatch ? Math.max(1, voiceNumberFromText(qtyMatch[1])) : 1;
    addMerged({kind:'match', drink, qty, text:bestAlias, mode});
  });
