...
let __bmoSpeechRetry = 0;
let __bmoVoicesReady = false;

function loadBmoVoices(){
  try{
    if(!('speechSynthesis' in window)) return [];
    const voices = window.speechSynthesis.getVoices() || [];
    if(voices.length) __bmoVoicesReady = true;
    return voices;
  }catch(_e){ return []; }
}
...
function pickBmoVoice(){
  try{
    ...
    return voices.find(v => /samantha|google us english|alex|ava|victoria|english/i.test(v.name || ''))
...
function runBmoSpeechQueue(){
  try{
    ...
    u.onstart = () => { __bmoSpeechRetry = 0; };
    u.onend = () => { __bmoSpeaking = false; setTimeout(runBmoSpeechQueue, 120); };
    u.onerror = () => {
      __bmoSpeaking = false;
      if(__bmoSpeechRetry < 2){
        __bmoSpeechRetry += 1;
        __bmoSpeechQueue.unshift(next);
        setTimeout(runBmoSpeechQueue, 250);
        return;
      }
      setTimeout(runBmoSpeechQueue, 120);
    };
    setTimeout(() => {
      try{ window.speechSynthesis.resume(); }catch(_e){}
      try{ window.speechSynthesis.cancel(); }catch(_e){}
      try{ window.speechSynthesis.speak(u); }catch(_e){ __bmoSpeaking = false; setTimeout(runBmoSpeechQueue, 120); }
    }, 80);
...
if('speechSynthesis' in window){
  try{
    window.speechSynthesis.onvoiceschanged = () => { loadBmoVoices(); };
    loadBmoVoices();
  }catch(_e){}
}
