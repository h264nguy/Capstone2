...
function parseVoiceDrinkSegments(text, mode='add'){
  ...
  parts.forEach(part => {
    ...
    if(!cleaned) return;
    let qty = voiceNumberFromText(cleaned);
    const drink = findDrinkFromVoice(cleaned);
    const suggestion = !drink ? findDrinkSuggestion(cleaned) : null;
    if(qty === 1 && (drink || suggestion)) qty = inferQtyNearPhrase(normalized, cleaned);
  ...
  if(out.length <= 1){
    ...
    (drinks || []).forEach(drink => {
      buildDrinkVoiceAliases(drink).forEach(alias => {
        ...
        const re = new RegExp(`(^|\\s)${escapeRegex(target)}(?=\\s|$)`, 'i');
        const m = normalized.match(re);
        if(m){
          const idx = m.index + (m[1] ? m[1].length : 0);
          matches.push({idx, drink, qty: inferQtyNearPhrase(normalized, target), text: target, mode, kind:'match'});
        }
      });
    });
    const dedup = [];
  ...
  if(segments.length > 1){
    ...
    if(!unknown.length && !suggestions.length){
      ...
      if(status){
        ...
        status.innerHTML = `${verb} <strong>${summarizeVoiceActions(segments, verb.toLowerCase())}</strong>.<div style="margin-top:8px;opacity:.75;font-size:12px">Heard: ${heardRaw}</div>`;
      }
      bmoSay(`${multiMode === 'remove' ? 'Removed' : 'Added'} multiple drinks`);
      return true;
    }
