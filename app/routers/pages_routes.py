...
function simplifyVoicePhrase(s){
  return normalizeVoiceText(s)
    .replace(/(hey|bmo|please|can|you|i|want|would|like|get|me|a|an|the|to|add|order|make|give|put|in|cart|for|now)/g, ' ')
    .replace(/check\s*out/g, ' checkout ')
    .replace(/fishes?/g, ' fizz ')
    .replace(/face/g, ' fizz ')
    .replace(/fees/g, ' fizz ')
    .replace(/fits/g, ' fizz ')
    .replace(/fists/g, ' fizz ')
    .replace(/fizzy/g, ' fizz ')
    .replace(/voltige/g, ' voltage ')
    .replace(/sun set/g, ' sunset ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function bestDrinkVoiceCandidate(phrase){
  ...
  const qs = simplifyVoicePhrase(phrase);
  if(!q && !qs) return {drink:null, score:0};
  let best = null;
  let bestScore = 0;
  (drinks || []).forEach(d => {
    ...
    aliases.forEach(alias => {
      ...
      const aliasSimple = simplifyVoicePhrase(alias);
      if(!aliasNorm && !aliasSimple) return;
      let score = 0;
      if((q && aliasNorm && (q.includes(aliasNorm) || aliasNorm.includes(q))) || (qs && aliasSimple && (qs.includes(aliasSimple) || aliasSimple.includes(qs)))){
        score = 0.98;
      } else {
        const sim1 = normalizedSimilarity(q, aliasNorm);
        const sim2 = normalizedSimilarity(qs, aliasSimple);
        const overlap1 = tokenOverlapScore(q, aliasNorm);
        const overlap2 = tokenOverlapScore(qs, aliasSimple);
        score = Math.max(sim1, sim2, overlap1 * 0.92, overlap2 * 0.96, (sim2 * 0.75) + (overlap2 * 0.25));
      }
      if(score > bestScore){
        bestScore = score;
        best = d;
...
function showVoiceSuggestion(drink, qty, action){
  const status = document.getElementById('status');
  if(!status || !drink) return;
  const safeAction = action === 'remove' ? 'remove' : 'add';
  const safeQty = Math.max(1, parseInt(qty || 1, 10));
  status.innerHTML = `Did you mean <button type="button" class="secondary" style="padding:6px 10px; margin-left:6px" onclick="confirmVoiceSuggestion('${drink.id}', ${safeQty}, '${safeAction}')">${drink.name}</button>?`;
}
...
function findDrinkFromVoice(phrase){
  ...
  return match.score >= 0.68 ? match.drink : null;
}

function findDrinkSuggestion(phrase){
  const match = bestDrinkVoiceCandidate(phrase);
  if(match.score >= 0.52 && match.score < 0.68) return match.drink;
  return null;
}
...
function handleVoiceCommand(rawText){
  ...
  if(/\b(remove|delete|take away|take out)\b/.test(text)){
    if(drink){
      ...
      showVoiceSuggestion(suggestion, qty, 'remove');
      bmoSay(`Did you mean ${suggestion.name}`);
    }else{
  ...
  if(/\b(add|order|make|get|want|like)\b/.test(text)){
    if(drink){
      ...
      showVoiceSuggestion(suggestion, qty, 'add');
      bmoSay(`Did you mean ${suggestion.name}`);
    }else{
      if(status) status.innerText = 'I could not match that drink name.';
      bmoSay('I could not find that drink');
