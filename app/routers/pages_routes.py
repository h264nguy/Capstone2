...
function addDrinkByVoice(drink, qty){
  ...
  const status = document.getElementById('status');
  if(status) status.innerHTML = `Added <strong>${qty} ${drink.name}${qty>1?'s':''}</strong>. Say checkout when ready.`;
...
function removeDrinkByVoice(drink, qty){
  ...
  const status = document.getElementById('status');
  if(status) status.innerHTML = `Removed <strong>${qty} ${drink.name}${qty>1?'s':''}</strong>.`;
...
function parseVoiceDrinkSegments(text, mode='add'){
  ...
  parts.forEach(part => {
    ...
    if(!cleaned) return;
    const qty = voiceNumberFromText(cleaned);
    const drink = findDrinkFromVoice(cleaned);
    const suggestion = !drink ? findDrinkSuggestion(cleaned) : null;
    if(drink){
      out.push({kind:'match', drink, qty, text: cleaned, mode});
    }else if(suggestion){
      out.push({kind:'suggestion', drink: suggestion, qty, text: cleaned, mode});
    }else{
      out.push({kind:'unknown', qty, text: cleaned, mode});
    }
  });
  return out;
}
