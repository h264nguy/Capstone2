...
let __bmoVoiceReady = false;
let __bmoVoiceLoaderBound = false;

function loadBmoVoices(){
  try{
    if(!('speechSynthesis' in window)) return [];
    const voices = window.speechSynthesis.getVoices() || [];
    if(voices.length) __bmoVoiceReady = true;
    if(!__bmoVoiceLoaderBound){
      __bmoVoiceLoaderBound = true;
      window.speechSynthesis.onvoiceschanged = () => {
        try{
          const loaded = window.speechSynthesis.getVoices() || [];
          if(loaded.length) __bmoVoiceReady = true;
          if(__speechUnlocked) setTimeout(runBmoSpeechQueue, 0);
        }catch(_e){}
      };
    }
    return voices;
...
function pickBmoVoice(){
  try{
    const voices = loadBmoVoices();
    return voices.find(v => /samantha|google us english|alex|ava|victoria|english/i.test(v.name || '')) || voices.find(v => /^en/i.test(v.lang || '')) || null;
  }catch(_e){ return null; }
}

function runBmoSpeechQueue(){
  try{
    if(__bmoSpeaking || !__bmoSpeechQueue.length || !('speechSynthesis' in window) || !__speechUnlocked) return;
    __bmoSpeaking = true;
    ...
    setTimeout(() => {
      try{ window.speechSynthesis.resume(); }catch(_e){}
      try{ window.speechSynthesis.speak(u); }catch(_e){ __bmoSpeaking = false; setTimeout(runBmoSpeechQueue, 180); }
    }, __bmoVoiceReady ? 30 : 150);
...
function unlockBmoSpeech(){
  ...
  try{
    ...
    warmup.pitch = 1;
    const preferred = pickBmoVoice();
    if(preferred) warmup.voice = preferred;
    window.speechSynthesis.speak(warmup);
  }catch(_e){}
  setTimeout(runBmoSpeechQueue, 60);
}
...
document.addEventListener('visibilitychange', () => {
  if(document.visibilityState === 'visible' && __speechUnlocked){
    try{ window.speechSynthesis.resume(); }catch(_e){}
    setTimeout(runBmoSpeechQueue, 0);
  }
});
