...
.audio-float{position:fixed;left:18px;bottom:18px;z-index:100000;display:flex;align-items:center;gap:10px;flex-wrap:wrap;max-width:min(420px,90vw);padding:10px 12px;border-radius:16px;background:rgba(0,0,0,.35);border:1px solid rgba(255,255,255,.14);backdrop-filter:blur(8px)}
.audio-btn{appearance:none;border:1px solid rgba(80,160,255,.70);background:rgba(10,18,28,.92);color:#f5e6d3;border-radius:12px;padding:10px 14px;cursor:pointer;font-weight:800;letter-spacing:.02em}
.audio-btn.active{border-color:rgba(80,160,255,.98)!important;box-shadow:0 0 0 3px rgba(80,160,255,.35),0 0 18px rgba(80,160,255,.45)}
.audio-badge{display:inline-block;padding:5px 10px;border-radius:999px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.05);font-size:12px;color:#f5e6d3}
.audio-hint{font-size:13px;opacity:.88;color:#f5e6d3}
@media (max-width:700px){.audio-float{left:14px;right:14px;bottom:14px;max-width:none}}
...
<div id='menu' class='grid cards' style='margin-top:14px'></div>
  ...
  <div class='audio-float'>
    <button id='audioEnableBtn' class='audio-btn' type='button'>Enable Audio</button>
    <div class='audio-hint'>Chrome needs one tap before BMO can speak. <span id='audioBadge' class='audio-badge'>Locked</span></div>
  </div>
...
function updateAudioUi(ready){
  ...
  if(btn){
    ...
    btn.textContent = ready ? 'Audio Ready' : 'Enable Audio';
  }
  if(badge) badge.textContent = ready ? 'Ready' : 'Locked';
}

function refreshBmoVoices(){
...
function pickBmoVoice(){
  try{
    const voices = refreshBmoVoices();
    return voices.find(v => /google us english|samantha|alex|ava|victoria|karen|moira/i.test(v.name || '')) || voices.find(v => /^en/i.test(v.lang || '')) || null;
  }catch(_e){ return null; }
}

function primeBmoSpeech(){
  ...
  try{
    ...
    u.rate = 1;
    u.pitch = 1;
    u.onend = () => { try{ window.speechSynthesis.cancel(); }catch(_e){} };
...
function runBmoSpeechQueue(){
  try{
    ...
    if(!__speechUnlocked){ updateAudioUi(false); return; }
    __bmoSpeaking = true;
    ...
    u.onerror = () => { __bmoSpeaking = false; setTimeout(runBmoSpeechQueue, 180); };
    setTimeout(() => {
      try{ window.speechSynthesis.resume(); }catch(_e){}
      try{ window.speechSynthesis.speak(u); }catch(_e){ __bmoSpeaking = false; }
    }, 90);
...
window.addEventListener('load', () => {
  ...
  try{
    if('speechSynthesis' in window){
      refreshBmoVoices();
      window.speechSynthesis.onvoiceschanged = () => { refreshBmoVoices(); if(__speechUnlocked) updateAudioUi(true); };
    }
  }catch(_e){}
  const btn = document.getElementById('audioEnableBtn');
  if(btn) btn.addEventListener('click', unlockBmoSpeech);
});
