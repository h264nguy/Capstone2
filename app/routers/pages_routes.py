...
function showVoiceSuggestion(drink, qty, action, heardText=''){
  ...
  const heardSafe = String(heardText || '').replace(/[<>&]/g,'');
  const heard = heardSafe ? `<div style="margin-top:8px;opacity:.82;font-size:12px"><strong>I heard:</strong> "${heardSafe}"</div>` : '';
  const meaning = `<div style="margin-top:8px;font-size:13px"><strong>Did you mean:</strong> <button type="button" class="secondary" style="padding:8px 12px; margin-left:6px" onclick="confirmVoiceSuggestion('${drink.id}', ${safeQty}, '${safeAction}')">${drink.name}</button></div>`;
  const hint = `<div style="margin-top:6px;opacity:.7;font-size:11px">Tap the drink name to confirm.</div>`;
  status.innerHTML = `${heard}${meaning}${hint}`;
}
...
function handleVoiceCommand(rawText){
  ...
  if(cmd === 'clear_cart'){
    cart = [];
    rerenderAfterVoice();
    if(status) status.innerText = `Heard: ${heardRaw} → cart cleared`;
    bmoSay('Cart cleared');
    return true;
  }
  if(cmd === 'history'){
  ...
  if(status) status.innerHTML = `<div><strong>I heard:</strong> "${String(heardRaw).replace(/[<>&]/g,'')}"</div><div style="margin-top:6px;opacity:.75">No close drink match found.</div>`;
