...
_ACTIVITY_VISIBLE_SECONDS = 5

def _recent_activity_items(raw):
    try:
        from datetime import datetime, timezone, timedelta
        cutoff = datetime.now(timezone.utc) - timedelta(seconds=_ACTIVITY_VISIBLE_SECONDS)
        items = []
        for it in (raw or []):
        ...
        return [it for it in (raw or []) if isinstance(it, dict)][:12]


@router.get('/api/activity')
def api_activity_feed():
    raw = _recent_activity_items(load_activity_log() or [])
    items = []
    for it in raw[:12]:
        if not isinstance(it, dict):
            continue
...
const ACTIVITY_VISIBLE_MS = 5000;

function parseActivityTs(ts){
  try{
    if(!ts) return null;
    const d = new Date(ts);
...
function formatActivityLocalTime(ts){
  const d = parseActivityTs(ts);
  if(!d) return '';
  try{ return d.toLocaleTimeString([], {hour:'numeric', minute:'2-digit'}); }catch(_e){ return ''; }
}

function pruneRecentActivity(items){
  const now = Date.now();
  return (Array.isArray(items) ? items : []).filter(item => {
    const d = parseActivityTs(item && item.ts);
    return d && (now - d.getTime()) <= ACTIVITY_VISIBLE_MS;
  }).slice(0, 6);
}
...
function startSharedActivityRefresh(){
  ...
  loadSharedActivity();
  activityPollTimer = setInterval(() => { if(document.hidden) return; loadSharedActivity(); renderActivity(); }, 1000);
}
...
function renderActivity(){
  ...
  if(!list) return;
  activityLog = pruneRecentActivity(activityLog);
...
function pushActivity(type, drinkName, qty){
  ...
  activityLog.unshift({
    ...
    qty: safeQty,
    ts: nowIso,
    timeLabel: formatActivityLocalTime(nowIso)
  });
  activityLog = pruneRecentActivity(activityLog);
  renderActivity();
