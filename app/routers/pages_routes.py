...
function bmoSay(text, flush=false){
  try{
    if(!('speechSynthesis' in window)) return;
    const msg = String(text || '').trim();
    if(!msg) return;
    if(flush){
      __bmoSpeechQueue = [msg];
      __bmoSpeaking = false;
...
function unlockBmoSpeech(){
  ...
  try{
    ...
    setTimeout(() => { try{ window.speechSynthesis.cancel(); window.speechSynthesis.resume(); }catch(_e){} }, 220);
...
function parseVoiceByOccurrences(text, mode='add'){
  ...
  const dedup = [];
  const seen = new Set();
  matches.sort((a,b) => a.idx - b.idx || a.drink.name.localeCompare(b.drink.name));
  matches.forEach(item => {
    const key = `${item.idx}:${item.drink.id}`;
...
function parseVoiceDrinkSegments(text, mode='add'){
  ...
  parts.forEach(part => {
    ...
    const withoutQty = cleaned.replace(/^(?:\d+|one|a|an|two|three|four|five|six|seven|eight|nine|ten)\b\s*/i, '').trim();
    const candidateText = withoutQty || cleaned;
    let qty = explicitQty || 1;
    const drink = findDrinkFromVoice(candidateText) || (!withoutQty ? null : findDrinkFromVoice(cleaned));
    const suggestion = !drink ? (findDrinkSuggestion(candidateText) || (!withoutQty ? null : findDrinkSuggestion(cleaned))) : null;
    if(!explicitQty && (drink || suggestion)) qty = inferQtyNearPhrase(normalized, candidateText || cleaned);
    if(drink) out.push({kind:'match', drink, qty, text: candidateText || cleaned, mode});
    else if(suggestion) out.push({kind:'suggestion', drink: suggestion, qty, text: candidateText || cleaned, mode});
    else out.push({kind:'unknown', qty, text: candidateText || cleaned, mode});
  });
  return out;
}
...
function mergeVoiceDrinkSegments(items){
  const merged = [];
  const byKey = new Map();
  (items || []).forEach(item => {
    if(!item || item.kind !== 'match' || !item.drink) return;
    const key = `${item.mode || 'add'}:${item.drink.id}`;
    if(!byKey.has(key)){
      const copy = {...item, qty: Math.max(1, parseInt(item.qty || 1, 10))};
      byKey.set(key, copy);
      merged.push(copy);
    } else {
      byKey.get(key).qty += Math.max(1, parseInt(item.qty || 1, 10));
    }
  });
  return merged;
}

function processVoiceDrinkMatches(items, heardRaw, mode){
  const status = document.getElementById('status');
  const merged = mergeVoiceDrinkSegments(items);
  ...
  if(status){
    ...
    const summary = summarizeVoiceActions(merged, verb.toLowerCase());
    status.innerHTML = `${verb} <strong>${summary}</strong>.<div style="margin-top:8px;opacity:.75;font-size:12px">Heard: ${heardRaw}</div>`;
  }
  bmoSay(`${mode === 'remove' ? 'Removed' : 'Added'} ${summarizeVoiceActions(merged, mode)}`);
  return true;
}
...
function handleVoiceCommand(rawText){
  ...
  if(segments.length){
    ...
    if(matches.length && !unknown.length && !suggestions.length){
      return processVoiceDrinkMatches(matches, heardRaw, multiMode);
    }
    if(suggestions.length && !matches.length && status){
