...
def _fmt_activity_time(ts: str | None) -> str:
    if not ts:
        return ''
    try:
...
def api_activity_feed():
    ...
    for it in raw[:12]:
        ...
        items.append({
            ...
            'ts': it.get('ts'),
            'timeLabel': _fmt_activity_time(it.get('ts')) or '',
        })
...
def api_activity_push(body: ActivityBody):
    ...
    return JSONResponse({'ok': True, 'item': {
        ...
        'qty': entry.get('qty'),
        'ts': entry.get('ts'),
        'timeLabel': _fmt_activity_time(entry.get('ts')) or '',
    }})
...
function bmoSay(text, flush=false, fromGesture=false){
  try{
    ...
    if(!msg) return;
    if(fromGesture){
      __lastGestureAt = Date.now();
      if(speakImmediately(msg)) return;
    }
    if(flush){
...
function noteUserGesture(){ __lastGestureAt = Date.now(); unlockBmoSpeech(); }
...
function startSharedActivityRefresh(){
  ...
  loadSharedActivity();
  activityPollTimer = setInterval(() => { if(!document.hidden) loadSharedActivity(); }, 1200);
}
...
async function checkout(btnEl) {
  ...
  const cupReady = (() => { try { return localStorage.getItem('tipsyCupReady') === 'true'; } catch(_e) { return false; } })();
  if (!cupReady) {
...
// UI feedback
  ...
  try {
    ...
    if(data && data.ok){
      ...
      try { localStorage.setItem('tipsyCupReady', 'false'); } catch(_e) {}
      cart = {};
      renderCart();
