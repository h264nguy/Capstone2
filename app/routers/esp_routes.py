...
def esp_next(key: str):
    ...
    if state.get("flush_required"):
        return {"ok": True, "order": None, "waitingForFlush": True, "flushRequired": True, "flushRequested": bool(state.get("flush_requested"))}
...
def esp_complete(body: CompleteBody, key: str):
    ...
    if ok:
        save_machine_state({"flush_required": True, "flush_requested": False, "last_completed_order_id": body.id})
...
def flush_request():
    ...
    save_machine_state({"flush_requested": True})
...
def esp_flush_complete(body: FlushCompleteBody, key: str):
    _check_key(key)
    save_machine_state({"flush_required": False, "flush_requested": False})
    return {"ok": True}
